...
def hoem1():
    ...
    conn.execute('SELECT balance FROM company WHERE companyname ="Namma Kadai"')
    user_data1 = conn.fetchone()
    conn.close()
    return render_template('index.html', balance=user_data1[0])
...
def home():
    if request.method == "get":
        ...
        user_data1 = conn.fetchone()
        conn.close()
        return render_template('index.html',balance=user_data1)
    else:
        ...
        if passw == "root" and c_name=="root":
            balance=user_data1[0]
...
def add_item():
        ...
        if total_purchase_amount>prebalance:
            ...
            conn.execute('INSERT INTO purchase (itemid, priceperitem, totalprice, pretotal, balance,Itemname) VALUES (%s, %s, %s, %s, %s, %s)', (item_id, price_per_item, total_purchase_amount, prebalance, balance, item_name))
    ...
    if request.method == "POST":
        ...
        if item_data:
            ...
            if item_quantity > available_quantity:
                ...
                conn.execute('INSERT INTO sales (itemid, soldpriceperitem, totalsoldprice, pretotals, balances,Itemname) VALUES (%s, %s, %s, %s, %s,%s)',
                             (item_id, price_value, total_sale_amount, prebalance, balance,item_name))
                conn.execute('UPDATE company SET balance = %s WHERE companyid = 1', (balance,))
                conn.connection.commit()
...
def purchase_report():
    ...
    conn.close()
    return render_template('purchasereport.html', purchases=purchases)
...
def add():
    ...
    if request.method == "POST":
        ...
        conn = mysql.connection.cursor()
        conn.execute('UPDATE `myshop`.`company` SET `balance` = `balance` + %s WHERE `companyname` = %s',
                       (amount, c_name))
        conn.connection.commit()
        msg="success"
        return render_template('add.html', cname=cname,msg=msg)
    else:
        return render_template('add.html',cname=cname)
